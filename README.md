...
## Day 76（已完成）：键盘 Smg 重绑 + 手柄按钮落盘

Settings Controls 补 `weapon_smg` 键盘重绑，并给 Dash / 四把枪加 Pad 列，按钮号写入 `[gamepad]`。左杆走速、右杆瞄准、RT/RB 开火仍硬编码。Joy 事件不准进 InputMap。Autoload 仍为 0。协议仍 4。Solo / Infinite / LAN Co-op / Battle 与 Day 75 相同。

- **键盘**：`REBINDABLE_ACTIONS` 末尾加 `"weapon_smg"`，`DEFAULT_KEYS["weapon_smg"]=KEY_4`，`action_display_name` = `"Smg"`。旧 cfg 缺 `controls/weapon_smg` 当 4，不改写已有 WASD/1/2/3。`_apply_key_bindings` 仍 erase 后只加 `InputEventKey`；`ARROW_FALLBACK` 仍只四向。
- **手柄字典**：独立 `_joy_overrides`，不要和 `_key_overrides` 混。`REBINDABLE_JOY_ACTIONS` = dash / pistol / shotgun / rifle / smg。默认 A + 十字键左/上/右/下。钥匙节 `[gamepad]`，存 int 按钮号。旧 cfg 缺节/缺键当默认，不改写 `[controls]`。`set_joy_button_for_action` 拒 `START`/`GUIDE`、拒表内同号、等于默认则 erase；setter 里不 save / apply。`joy_label_for_action` 用 Xbox 短名（Godot 4.6 没有 `Input.get_joy_button_string`），空则 `"%d"`。`save_to_disk` 五键都写出当前值。`apply()` 仍只 `_apply_key_bindings`。
- **Settings Controls**：每行 HBox `separation=12`；四向只有键盘钮；Dash 和四把枪再一颗 Pad `OfferButton` 160×44。Body 顶 `RunSummaryHint`：「Sticks stay analog. Pad column rebinds Dash and guns.」点 Pad 听 `InputEventJoypadButton.pressed`，忽略 Motion / 鼠标 / 扳机轴。Esc / `ui_cancel` 取消。`START`/`GUIDE` 闪 IN USE。成功 `save_to_disk` 刷新该钮，不必改 InputMap。失败闪 IN USE 0.6s。搜索：键盘行仍 `action_display_name`；Pad 钮 meta `"gamepad joypad pad dash gun"`。不要第五栏、不要 Restore Defaults。
- **PlayerInput**：Dash / 切枪 / `_joy_wants_control` 读当前绑定。四把枪边沿用 `PackedByteArray`。左杆 / 右杆 / RT/RB 一字不改。远程 Guest 仍 `apply_remote_frame`。

**当时不做：** Restore Defaults、虚拟摇杆、导出、第五栏 Gamepad、5 人/P2P、主动技能、改协议 4、改 SNAPSHOT_VERSION、改战斗数字、新 wav、新 PNG、新 CanvasLayer、Autoload、把 Joy 写进 `move_*` / `dash` / `weapon_*`、重绑摇杆轴/扳机轴/开火、把 Start/Guide 绑成 Dash 或切枪。


## Day 77（已完成）：Controls 打磨

冲突从只闪 IN USE 升级成旁白 + Error。RESTORE DEFAULTS 只清键位。商店 PICK_GUN 跟手柄枪绑定走。Start 仍是系统键。Autoload 仍为 0。协议仍 4。Solo / Infinite / LAN Co-op / Battle 与 Day 76 相同。

- **GameSettings**：`reset_controls()` 只 `_key_overrides.clear()` 与 `_joy_overrides.clear()`，不 apply、不 save、不动音量/显示。`find_key_conflict` 表内其它动作同键返回其 action id，否则 `""`。`find_joy_conflict`：START/GUIDE → `"reserved"`；越界或不在 `REBINDABLE_JOY_ACTIONS` → `"invalid"`；表内其它同号 → 那个 action id；否则 `""`。`set_key_for_action` / `set_joy_button_for_action` 仍只 return false，不拼句子。不新开 settings.cfg 节。
- **Settings Controls**：保留 analog hint。hint 下 `BindStatus`（RunSummaryHint，IGNORE，默认空）。冲突钮仍 "IN USE" 0.6s；reserved 旁白 `"Start is reserved."` / `"Guide is reserved."`；占用 `"%s is used by %s."`（键盘 `OS.get_keycode_string`，手柄 `joy_label`）；invalid 旁白空仍闪 IN USE。成功 / Esc / `open()` 清空旁白。冲突播 Error（`click.wav` 走 SFX，`_sfx_gate`），不播 click。Body 底 `RESTORE DEFAULTS` OfferButton 高 56：`reset_controls` + apply + save + 刷新钮 + `"Controls restored."` + click。搜索 meta `"restore defaults reset keys gamepad"`。听 Pad 按 Start 走冲突 UI，不关 Settings。摇杆/扳机仍忽略。不要第五栏。
- **ShopOffer PICK_GUN**：删写死四向 DPAD。对 `REBINDABLE_JOY_ACTIONS` 里四把枪（跳过 dash）用 `get_joy_button_for_action` 比对 → 槽 0..3。Start 仍 Back。浏览态 Joy 除 Start 外不买货。`_process` 仍可读 PlayerInput 槽。三选一十字键左/上/右仍点卡 0/1/2，不跟枪绑定走。
...
## Day 79（已完成）：出生点 1～5 十字 + Co-op 3 人能打

删掉「所有 Guest 叠在 (80,0)」路径。座位 1～5 共用一张世界坐标十字表，三图同一套，Retry 回各自点。相机仍只跟 `_local_player`。2 人 Co-op / Battle 出生仍 (0,0)/(80,0)，与 Day 78 无法分辨。Autoload 仍为 0。协议仍 5。LAN 仍不写档。

- **SEAT_SPAWNS**：`(0,0)` / `(80,0)` / `(-80,0)` / `(0,80)` / `(0,-80)`。`GUEST_SPAWN` 只当 seat 2 别名。`_spawn_for_seat` clamp 1..5。相邻中心距 80。不按 `arena_id` 分表，不探测墙再挪。
- **CombatSandbox**：seat 1 场景 Player 仍 (0,0)，不改 tscn。每个占用 seat≥2 instantiate 后 `global_position` 与 `set_spawn_position` 都走表。`_ensure_guest_pawn` 同样，禁止先弹去 (80,0) 再等包。`_reset_sandbox` 仍 `reset_for_sandbox()` 回 `_spawn_position`。`_bind_enemies` 把 `_synced_pawns` 交给 `bind_players`。HUD 左下仍本地；Battle RivalRow 仍第一只非本地。相机仍 `bind_player(_local_player)`。
- **Co-op 3 人**：进场站开，能走能开枪（Host 权威）。近战追三人里最近活 pawn。P8 目录连买；第三人买 Pack S / Gunner 打买家。共享一份 RunSession。一人倒下其余继续；全灭才 Winner。玩家弹仍打不中队友。Battle Start 仍必须恰好 2。第四、第五人若进房，出生 `(0,80)` / `(0,-80)`，不叠柱。
- **DebugOverlay**：`p3_hp` 读 seat 3，没有则 `-`。不要为 4/5 新开 HUD 条。`bind_p2` 仍只绑 seat 2。

**当时不做：** 3～5 人短血 HUD、房间浏览器、Battle FFA、不改协议 5、不改 SNAPSHOT_VERSION、不改 `rpc_begin` 签名、不改战斗数字、不写档、新 PNG、新 wav、新物理层、新 InputMap action、Autoload。

## Day 80（已完成）：3～5 人右上 Roster 短血

占用 ≥3 的 LAN Co-op，右上给每个非本地座位一条短血。左下仍只绑 `_local_player`（280×16）。2 人 Co-op / 2 人 Battle / Solo 的 HUD 与 Day 79 无法分辨。Autoload 仍为 0。协议仍 5。出生点仍十字。LAN 仍不写档。

- **Hud Roster**：`TopRight` `offset_bottom=176`。`RivalRow` 一字不改（200×12，文案 `"rival  n/m"`）。其下预放 `Roster` VBox（separation=6，默认 hidden）+ Row1..Row4，条 160×8，文案 `"s%d  %d/%d"`。Co-op 占用 < 3：Roster hidden。占用 ≥ 3：按 seat 升序显示非本地短血，最多 4 条。Battle 占用==2：仍 RivalRow，Roster hidden。Solo 右上没有任何血条。倒下的行仍在，血 0、`modulate.a=0.45`。低血换皮只留左下本地条。
- **CombatSandbox**：`_bind_party_hud` 统一绑 Rival + Roster + Overlay pawns；`_ready` / `_rebind_after_pawn_change` / `_convert_lan_host_to_solo` 都走这里。大厅掉线 / `_remove_seat_pawn` 后重绑；占用掉到 2 则 Roster 收掉。相机仍只跟 `_local_player`。
- **DebugOverlay**：删 `p2_hp` / `p3_hp`。`seat_hp` 读座位 1～5，空位 `-`（例三人 `100,100,80,-,-`）。`seats:` 行保留。
...
## Day 82（已完成）：Co-op 4～5 能打完一场

4 人、5 人 LAN Co-op Pit 能进场、能打、能开 P8 连买、能 Retry。出生仍十字，不叠柱。金币 / XP / owned 仍一份，五端同一数字。`COMPANION_CAP` 仍 10，全队共享。Stim 本店一次、只护买家。2 人 / 3 人 Co-op 与 2 人 Battle / 房间列表与 Day 81 无法分辨。Autoload 仍为 0。协议仍 5。LAN 仍不写档。F7 / F8 在 LAN 仍无效。

- **出生**：`_prepare_pawns` / `_ensure_guest_pawn` 一律 `_spawn_for_seat`。seat 4 `(0,80)` / seat 5 `(0,-80)`。`GUEST_SPAWN` 仍只是 seat 2 别名。Retry / `reset_for_sandbox` 回各自 `_spawn_position`。
- **商店买家**：Guest `send_try_shop`，Host `_pawn_for_seat(seat)`。seat 4 与 5 能买。Host 本地点击仍 `_host_buyer()`=seat 1。共享金：seat 4 买 Pack S，五端 gold -20，只有 seat 4 血 +40。任意座位买 Gunner 占全队 10 只名额，`bind_owner`=买家。seat 5 买 Stim 只护 seat 5，本店 Stim 变灰。Pack L 仍 fill 买家 + 所有活跟班。
- **绑定**：`_synced_pawns` 跳过 null。敌人 / RunSession / Applier 用占用 pawn。鸡专属卡按在场角色过滤，五个座位里有鸡才出鸡卡，只给鸡 pawn。Applier 底值按 pawn 实例对齐，掉人后不把别人的底值套到 seat 5。
- **快照 / Guest**：v3 按 seat 对齐 4/5。缺的 instantiate，多的 `queue_free`；不删本地席。迟到的本地席先认 `_local_player` 再跟相机。
- **掉线**：仍有远程 peer 只摘那一席并 `_rebind_after_pawn_change`。5 人局里踢 seat 4，其余继续，Roster 收成 3 条，不要 toast 转单机。只剩 Host 才 “Guest left the room. Switched to solo.”
- **HUD**：5 人 = 左下本地 + 右上 4 条 Roster。4 人 = 3 条。3 人仍 2 条。2 人 Co-op 右上仍空。文案仍 `"s%d  %d/%d"`，条仍 160×8。Overlay `seat_hp` 1～5，空位 `-`。相机仍只跟 `_local_player`。
- **大厅**：Co-op Start 占用 2～5。房间卡 occupied>=5 仍灰。Host 选 Battle 且已有 4 人：Start 灰。手打 IP 仍为主。不改 LanOverlay。弹池仍 `POOL_CAPACITY=96`。

**当时不做：** Battle FFA、不改协议 5、不改 SNAPSHOT_VERSION、不改 `rpc_begin` 签名、不改 `SEAT_SPAWNS`、不写档、不改数字（Gunner 140/520/70、Pack/Stim、`COMPANION_CAP=10`、Motor 420、`look_ahead=100`、10+4 卡 value）、不改 HUD 左下锚点 / Roster 160×8 / `ROSTER_MAX`、不分屏、不按 seat 拆金库、不做每人 10 只 Gunner、不投票关店、不改四把枪/敌人身份、Autoload、新 PNG、新 wav、新物理层、新 InputMap action。

## Day 83（已完成）：LAN Battle FFA 3～5

Battle Start 占用 2～5（与 Co-op 同一范围）。房间卡 Battle 满员改成占用≥5，2/5 与 3/5 的 Battle 卡可点进。先倒下的人留在座位当尸体旁观，镜头钉尸体，Winner 要等只剩 1 或 0。最后一人 KO，同帧全灭 DRAW。2 人 Battle 走同一套数活人数，与 Day 82 无法分辨。不写档、无句读、无商店、无跟班。Autoload 仍为 0。协议仍 5。LAN 仍不写档。F7 / F8 在 LAN 仍无效。

- **大厅**：`_can_start` Battle 不再要求 occupied==2；占用 2～5 且 Guest 都握手即可。`_is_room_full` 只看 occupied>=max_seats（5）。LoopRow 在 Battle 仍 disabled，标签仍 `"battle"`。Host 选 Battle 且 5 人房切 Battle：Start 可点。切回 Co-op 仍 5 人能开店。第六人仍被踢。
- **胜负**：Host `_resolve_battle_if_needed` 每帧数占用且 `not is_defeated` 的 pawn。占用 < 2 不结算。alive>=2 继续；==1 `_battle_winner_seat`=那人（1～5）；==0 DRAW（seat 0）。然后 `mark_battle_over()`。Guest 仍靠 `rpc_winner`。禁止走 `_all_pawns_defeated` 弹 Winner。2 人是同一函数，没有第二套 `if occupied==2`。
- **旁观**：现有 `is_defeated` + `on_defeated` 停走/停枪/停 Dash/停切枪。尸体留在 `SEAT_SPAWNS`，不 `queue_free`、不挪座位。相机仍 `PlayerCamera.bind_player(_local_player)`。本地倒下 `_aim_reticle.visible=false`，不 `_set_offer_input_lock(true)`。Pause / Esc 仍全员能开。
- **子弹**：`Projectile._handle_hit` 打到已 defeated 的 Player 直接 return：不火花、不回收、不伤，弹穿过还能打到后面的活人。活着的敌对 Player 仍 `apply_damage`。owner 忽略不变。Co-op `hit_players` 仍 false。
- **HUD**：2 人 Battle 仍 RivalRow `"rival  n/m"`，Roster hidden。Battle 占用≥3：RivalRow hidden，Roster 按 seat 升序非本地短血，文案仍 `"s%d  %d/%d"`，条仍 160×8，倒下 `modulate.a=0.45`。XP/gold 仍 hidden。Phrase 仍 `"BATTLE"`。Co-op HUD 一字不改：占用<3 右上空，≥3 Roster。
- **Winner / Retry**：winner_seat==0 DRAW；local==winner KO+ClearedTitle；否则 KO+RunSummaryTitle；record_name BATTLE；不写档。Host Retry 重置占用 pawn 体力/位置/枪，仍空场 Battle，人数不变。Guest Retry 仍禁用。
- **掉线**：仍有远程 peer 只摘那一席；Roster/胜负按剩下占用重算。只剩 Host 才 toast 转单机，不给自己 KO。

**当时不做：** P2P、不改协议 5、不改 SNAPSHOT_VERSION、不改 `rpc_begin` 签名、不改 `SEAT_SPAWNS`、不改数字（Gunner 140/520/70、Pack/Stim、`COMPANION_CAP=10`、Motor 420、`look_ahead=100`、10+4 卡 value）、不改 HUD 左下锚点 / Roster 160×8 / `ROSTER_MAX`、不分屏、不切镜头到赢家、不自由飞、不 queue_free 尸体、不把 FFA 做成积分制、Battle 仍无句读/商店/跟班/敌人、不写档、Autoload、新 PNG、新 wav、新物理层、新 InputMap action。

## Day 84（已完成）：2 人回归日

硬闸。两进程真跑 Pit Co-op 跟班+药 与 Yard Battle KO/DRAW。78–83 没有打坏 occupancy==2。两条一次过，gated 清单 1～7 都没修，没有改 gd/tscn。Autoload 仍为 0。`NET_PROTOCOL` 仍 5。`SNAPSHOT_VERSION` 仍 3。LAN 仍不写档。

- **Pit Co-op 占用 2**：Host Custom Pit + Co-op。出生 `(0,0)` / `(80,0)`。能走能开枪。Overlay：`play: coop`，`seats: 1,2 local=1`（Guest `local=2`），`seat_hp: 100,100,-,-,-`，`arena: pit`，`record: -`。HUD 左下本地条；RivalRow hidden；Roster hidden。玩家弹打不中队友（100→100）。打到 P8 目录连买：Host Pack S 金 136→116（-20），买家 70→110（+40），队友血不变；Guest 买 Gunner 步枪（`gun=1`），Host 侧 `puppet=false` 跑 AI，Guest 侧 `puppet=true`；Stim 只护买家 1.50s，队友 0。金币两端同一数字（店末 11）。不必 CLEARED。
- **Yard Battle 占用 2**：Host Custom Yard + Battle。场上无怪无店无跟班。Overlay：`play: battle`，`seats: 1,2 local=1|2`，`seat_hp: 100,100,-,-,-`，`arena: yard`，`record: -`。HUD RivalRow 可见、Roster hidden、Phrase `BATTLE`、XP/gold hidden。玩家弹打中对方（100→76）。先倒：赢方 `KO` + ClearedTitle，输方 `KO` + RunSummaryTitle，文案 `BATTLE`。Guest Retry disabled，Winner 仍开。Host Retry 两人回 `(0,0)` / `(80,0)`，仍 Yard Battle 空场。同帧双倒 `DRAW` + RunSummaryTitle。
- **写档**：这两局前后 `user://records.json` history 仍 12 条，字节不变。
...
- **tab 切换 = app 式水平翻页**：`MainMenu._switch_page()` 是唯一入口，`PAGE_ORDER`（home 0 / play 1 / records 2 / multiplayer 3 / profile 4 / ranking 5）算出 `direction = +1 / -1`。进出是**一条刚性纸带**：两页共用 `PAGE_SLIDE_SEC = 0.32` + 同一条 **easeOutCubic**（`TRANS_CUBIC` / `EASE_OUT`，起步快收尾缓；两页同曲线、只有位移方向相反 → 出页位移就是进页的镜像），位移严格互补（`|出页.x − 进页.x| == 页宽`，运动期逐帧实测偏差 ≤ 0.0002；曲线判别：out_cubic 残差 0.0014，其它候选曲线 0.06~0.58），方向翻页**不做内容淡入淡出**（纸带是实体）—— 所以一页进来时另一页必然同时在走，不会一快一慢或出现缝隙/重叠。只有 `direction = 0`（非 tab 的叠层、直接调用）才走旧的淡入 + 上浮。Home 舞台块由 `UiAnim.slide_in/slide_out` 一起滑，静止位置用 `page_base_x` meta 记住（Home 的父节点是 Control，不是 Container，布局不会把被移动过的 position 收回来）。`direction = 0` 仍是旧的淡入 + 上浮，给非 tab 的叠层与直接调用用。Settings 是 Drawer，不参与翻页。
- 页面自带的 Back / Esc 走 `close(-1)`（向右滑回）；Ranking 的 Back 改走 `MainMenu._on_profile_pressed()`（不再自己 `profile.open()`），保证 `MainMenu._page` 永远与屏幕一致 —— 实测状态机：home→play→multiplayer→profile→ranking→(Back)→profile→home 每一步 `_page` / `_home_slid_out` 都对，结束 `home.x = 0`；**顶栏 tab 一律导航**（在 Multiplayer 点 PLAY、在 Records 点 PLAY 都能切），翻页途中再点 tab 会排队到这一趟走完，避免出页从半路起步破坏刚性。
...
**Day 76 = 键盘 Smg 重绑 + 手柄按钮落盘（已完成）**：`weapon_smg` 进键盘重绑；`[gamepad]` 五键落盘；Pad 列听钮；PlayerInput Dash/切枪/认领读绑定；摇杆/扳机不重绑；Joy 不进 InputMap。
...
**Day 79 = 出生点 1～5 十字 + Co-op 3 人能打（已完成）**：`SEAT_SPAWNS` 十字、相机仍只跟本地；HUD 左下仍本地。LAN 仍不写档。协议仍 5。
...
**Day 82 = Co-op 4～5 能打完一场（已完成）**：出生仍十字，金共享，`COMPANION_CAP=10` 不改。seat 4/5 买对 pawn；掉一人 Roster 收，最后 Guest 才转单机。2 人 / 3 人 / 房间列表 / Battle 仅 2 人 Start 与 Day 81 相同。LAN 仍不写档。协议仍 5。

**Day 83 = Battle FFA 3～5（已完成）**：先倒旁观，最后一人 KO，同帧全灭 DRAW。2 人 Battle 与任意 Co-op 与 Day 82 无法分辨。LAN 仍不写档、无句读。协议仍 5。
