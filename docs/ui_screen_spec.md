...
- **翻页**：顶栏 tab 切换是水平翻页，不是淡入 —— `MainMenu._switch_page()` 按 `PAGE_ORDER` 算 `direction`；进出共用 0.32s 与同一条 easeOutCubic（`TRANS_CUBIC` / `EASE_OUT`）曲线（刚性纸带，`|出页.x − 进页.x| == 页宽` 恒定；曲线左右对称，出页即进页的镜像）；翻页途中点 tab 排队到本趟结束，方向翻页不淡入淡出，Home 舞台块一起滑；Settings 是 Drawer 不参与。页面自带 Back/Esc 用 `close(-1)`。
