...
var _hist_fade_tween: Tween
var _sfx_gate: Dictionary = {}
var _hist_rows: Array[HBoxContainer] = []
...
@onready var _sheet: Control = $Root/Sheet
@onready var _title: Label = $Root/Sheet/Column/Title
@onready var _record_name: Label = $Root/Sheet/Column/RecordName
@onready var _score_label: Label = $Root/Sheet/Column/Body/Left/Score
@onready var _new_best: Label = $Root/Sheet/Column/Body/Left/NewBest
@onready var _breakdown: VBoxContainer = $Root/Sheet/Column/Body/Left/Breakdown
@onready var _loop_label: Label = $Root/Sheet/Column/Body/Left/Breakdown/LoopRow/Label
@onready var _loop_value: Label = $Root/Sheet/Column/Body/Left/Breakdown/LoopRow/Value
@onready var _kills_label: Label = $Root/Sheet/Column/Body/Left/Breakdown/KillsRow/Label
@onready var _kills_value: Label = $Root/Sheet/Column/Body/Left/Breakdown/KillsRow/Value
@onready var _gold_label: Label = $Root/Sheet/Column/Body/Left/Breakdown/GoldRow/Label
@onready var _gold_value: Label = $Root/Sheet/Column/Body/Left/Breakdown/GoldRow/Value
@onready var _time_label: Label = $Root/Sheet/Column/Body/Left/Breakdown/TimeRow/Label
...
@onready var _retry_button: Button = $Root/Sheet/Column/Buttons/Retry
@onready var _menu_button: Button = $Root/Sheet/Column/Buttons/Menu
...
func present(record_id: String, session: RunSession, previous_best: int, winner_seat: int = 0, local_seat: int = 1, battle: bool = false) -> void:
	...
	UiAnim.kill_tween(_hist_fade_tween)
	_roll_done = false
...
func _fill_history(record: GameRecord) -> void:
	for row: HBoxContainer in _hist_rows:
		row.visible = false
...
func _play_score_roll() -> void:
	...
	if _is_battle_result:
		...
		return
	var beat: int = 0
	_append_break_roll(beat, _assign_loop_points, float(_final_loop * 1000), _loop_value)
	beat += 1
	_append_break_roll(beat, _assign_kills_points, float(_final_kills * 5), _kills_value)
	beat += 1
	_append_break_roll(beat, _assign_gold_points, float(_final_gold * 2), _gold_value)
	beat += 1
	_append_break_roll(beat, _assign_time_points, float(floori(_final_time_sec)), _time_value)
	beat += 1
...
func _play_history_fade() -> void:
	UiAnim.kill_tween(_hist_fade_tween)
	_hist_fade_tween = UiAnim.enter_stagger_fade(self, _history_fade_items())

func _history_fade_items() -> Array[CanvasItem]:
	var items: Array[CanvasItem] = [_rank_label]
...
func _assign_loop_points(value: float) -> void:
	_loop_value.text = str(roundi(value))

func _assign_kills_points(value: float) -> void:
	_kills_value.text = str(roundi(value))

func _assign_gold_points(value: float) -> void:
	_gold_value.text = str(roundi(value))

func _assign_time_points(value: float) -> void:
	_time_value.text = str(roundi(value))
...
func _write_break_texts(loop_pts: int, kill_pts: int, gold_pts: int, time_pts: int) -> void:
	_loop_label.text = "loop  %d  ×1000" % _final_loop
	_kills_label.text = "kills  %d  ×5" % _final_kills
	_gold_label.text = "gold  %d  ×2" % _final_gold
	_time_label.text = "time  %.1fs" % _final_time_sec
	_loop_value.text = str(loop_pts)
	_kills_value.text = str(kill_pts)
	_gold_value.text = str(gold_pts)
	_time_value.text = str(time_pts)
...
func _list_roll_fade_items() -> Array[CanvasItem]:
	var items: Array[CanvasItem] = [_loop_value, _kills_value, _gold_value, _time_value, _summary, _rank_label]
	if _cleared_bonus.visible:
