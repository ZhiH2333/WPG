...
func bind_arena_id(id: String) -> void:
	_arena_id = id
...
func bind_pawns(pawns: Array[Player]) -> void:
	_pawns = pawns

func bind_seats(occupied: PackedInt32Array, local_seat: int) -> void:
	var parts: PackedStringArray = PackedStringArray()
	for seat: int in occupied:
		parts.append(str(seat))
	if parts.is_empty():
		_seats_label = str(clampi(local_seat, 1, 5))
		return
	_seats_label = "%s local=%d" % [",".join(parts), local_seat]
...
func _has_chicken_present() -> bool:
	...
	for pawn: Player in _pawns:
		if pawn == null or not is_instance_valid(pawn):
			continue
		if pawn.get_character_id() == "chicken":
			return true
	return false
...
func _read_aim_stick_status_line() -> String:
	if _player_input == null:
		return ""
	var id: int = _player_input.get_device_id()
	if id < 0:
		return ""
	var raw: Vector2 = Vector2(Input.get_joy_axis(id, JOY_AXIS_RIGHT_X), Input.get_joy_axis(id, JOY_AXIS_RIGHT_Y))
	if PlayerInput.map_aim_stick(raw).is_zero_approx():
		return "aim: rest\n"
	return "aim: live\n"
...
func _read_seat_hp() -> String:
	...
	for seat: int in range(1, 6):
		...
		if index >= _pawns.size():
			parts.append("-")
			continue
		var pawn: Player = _pawns[index]
		if pawn == null or not is_instance_valid(pawn):
			parts.append("-")
			continue
		parts.append(str(pawn.get_player_health().get_hp()))
	return ",".join(parts)
